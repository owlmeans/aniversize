...
Scans the project (rooted at `root` or `cwd`) to detect which AI coding agents are configured. Uses characteristic files and folders as signals, weighted by proximity to the project root. Prints all detected agents and their signals, then writes `.aniversize/meta.json` with the identified primary agent.

Use `--dry` to skip writing `meta.json` and instead print `Would write .aniversize/meta.json`.

When `--dry` is not set and `-y` is not provided, the command prompts for confirmation before overwriting `meta.json` if it already exists with different content. Press `y` to overwrite, or Enter/`n` to skip. Use `-y` (`--yes`) to suppress all prompts and overwrite automatically.
...
The result is persisted in `.aniversize/meta.json`:

```json
{ "primary": "copilot" }
```

This file is intended to be read by the `generate` command (and future commands) to know which agent to target by default.
...
1. Determine the target agent:
   ...
   - Otherwise read `.aniversize/meta.json` for the `primary` field.
   - Otherwise run `identify` automatically, write `meta.json`, and use the detected agent.
...
src/
  ...
  converters/       — One module per agent: reads .aniversize files, writes agent output
tests/
...
| `aniversize identify [root]` | Detect primary AI agent and write `.aniversize/meta.json` |
